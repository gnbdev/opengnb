...
static void if_up_script(gnb_core_t *gnb_core) {
    int ret;
    char cmd[1024];
    snprintf(cmd, 1024, "\"%s/scripts/%s\" > /dev/null 2>&1", gnb_core->conf->conf_dir, "if_up_netbsd.sh");
    ret = system(cmd);
    if (-1 == ret || 0 == ret) {
        return;
    }
...
static void if_down_script(gnb_core_t *gnb_core) {
    int ret;
    char cmd[1024];
    snprintf(cmd, 1024, "\"%s/scripts/%s\" > /dev/null 2>&1", gnb_core->conf->conf_dir, "if_down_netbsd.sh");
    ret = system(cmd);
    if (-1 == ret || 0 == ret) {
        return;
    }
...
int init_tun_netbsd(gnb_core_t *gnb_core) {
    gnb_core->tun_fd = -1;
    return 0;
}

static int open_tun_netbsd(gnb_core_t *gnb_core) {
    ...
    int flags = fcntl(gnb_core->tun_fd, F_GETFD);
    flags |= FD_CLOEXEC;
    fcntl(gnb_core->tun_fd, F_SETFD, flags);

    /*
     * TUNSIFHEAD enables multi-af mode on NetBSD, where every packet is
     * preceded with a four-byte address family in network byte order.
     */
    int on = 1;
    if (ioctl(gnb_core->tun_fd, TUNSIFHEAD, &on) < 0) {
...
static int read_tun_netbsd(gnb_core_t *gnb_core, void *buf, size_t buf_size) {
    ssize_t rlen;
    uint32_t type;
    struct iovec iv[2];
    ...
    if (rlen - (ssize_t)sizeof(uint32_t) <= 0) {
        return -1;
    }
    return rlen - sizeof(uint32_t);
}

static int write_tun_netbsd(gnb_core_t *gnb_core, void *buf, size_t buf_size) {
    uint32_t type;
    struct iovec iv[2];
    ...
    return writev(gnb_core->tun_fd, iv, 2);
}

static int close_tun_netbsd(gnb_core_t *gnb_core) {
...
static int release_tun_netbsd(gnb_core_t *gnb_core) {
    return 0;
}

gnb_tun_drv_t gnb_tun_drv_netbsd = {
    init_tun_netbsd,
    open_tun_netbsd,
    read_tun_netbsd,
    write_tun_netbsd,
