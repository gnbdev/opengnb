...
static void detect_node_address(gnb_worker_t *gnb_detect_worker, gnb_node_t *node){
    ...
    //debug_text
    snprintf(detect_addr_frame->data.text,32,"[%"PRIu64"]FULL_DETECT[%"PRIu64"]", gnb_core->local_node->uuid64, node->uuid64);
    ...
    GNB_LOG4(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "Try node address [%"PRIu64"]->[%"PRIu64"]%s status=%d\n", gnb_core->local_node->uuid64, node->uuid64, GNB_IP_PORT_STR1(&address_st), node->udp_addr_status);
...
static void detect_loop(gnb_worker_t *gnb_detect_worker){
    ...
    for ( i=0; i<num; i++ ) {
        ...
        if ( (gnb_core->local_node->type & GNB_NODE_TYPE_SLIENCE) && !(node->type & GNB_NODE_TYPE_FWD) ) {
            GNB_LOG5(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "Skip Detect [%"PRIu64"]->[%"PRIu64"] node type is SLIENCE and not FWD\n", gnb_core->local_node->uuid64, node->uuid64);
            continue;
        }

        if ( ( GNB_NODE_STATUS_IPV6_PONG | GNB_NODE_STATUS_IPV4_PONG) & node->udp_addr_status ) {
            GNB_LOG5(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "Skip Detect [%"PRIu64"]->[%"PRIu64"] already P2P status=%d\n", gnb_core->local_node->uuid64, node->uuid64, node->udp_addr_status);
            continue;
        }
        ...
        if ( time_difference > GNB_DETECT_PUSH_ADDRESS_INTERVAL_SEC ) {
            GNB_LOG5(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "Skip Detect [%"PRIu64"]->[%"PRIu64"] status=%d time_difference(%"PRIu64") > GNB_DETECT_PUSH_ADDRESS_INTERVAL_SEC(%"PRIu64") last_push_addr_sec=%"PRIu64"\n",
        ...
        if ( full_detect_time_difference < gnb_core->conf->full_detect_interval_sec ) {
            ...
            GNB_LOG5(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "Skip Detect [%"PRIu64"]->[%"PRIu64"] status=%d full_detect_time_difference(%"PRIu64") < full_detect_interval_sec(%"PRIu64") last_full_detect_sec=%"PRIu64"\n",
        ...
        if ( gnb_core->conf->port_detect_start == node->detect_port4 ) {
            GNB_LOG3(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "#START FULL DECETE node[%"PRIu64"] idx[%d]\n", node->uuid64, node->detect_address4_idx);
        }
        ...
        if ( gnb_core->conf->port_detect_end == node->detect_port4 ) {
            ...
            GNB_LOG3(gnb_core->log, GNB_LOG_ID_DETECT_WORKER, "#END FULL DECETE node[%"PRIu64"] idx[%d]\n", node->uuid64, node->detect_address4_idx);
        }
